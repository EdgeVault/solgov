...
async function scanProtocol(
  ...
  // Step 3: Try Squads config FIRST on any authority (audit fix #1)
  // Vault PDAs are owned by System Program at the account level but controlled
  // by a Squads multisig underneath. If we check governance type first, we see
  // "System Program" and falsely flag as EOA. So: Squads first, governance fallback.
...
async function main() {
  ...
  console.log(`CRITICAL: ${critical.length} protocols`);
  console.log(`WEAK:     ${weak.length} protocols`);
