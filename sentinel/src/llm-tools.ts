 ...
 *
 * Design: each tool is a small pure-ish function with a JSON-serialisable
 * return value. Returns are capped in size so the LLM doesn't blow context.
 * The LLM decides which tools to invoke based on the alert it's triaging.
 *
 * OpenAI-compatible tool schema so it works with Groq (llama-3.3-70b) and
 * the Anthropic fallback. Registered via registerTools() + executeTool().
 */
...
// On the VPS the public-dashboard repo is not present, so we allow an env
// override that points at a local mirror (e.g. /home/kris/solgov/data/dashboard-mirror).
// Dev machines fall through to the sibling path.
...
// concludes routine, we stop. This mirrors how the user manually investigates:
  ...
  NEW_SIGNER: {
    ...
    steps: [
      ...
      },
      {
        id: 'cross_protocol',
        label: 'Known signer?',
        instruction: 'Does this address hold authority elsewhere on our tracked set. Is it a known team wallet.',
