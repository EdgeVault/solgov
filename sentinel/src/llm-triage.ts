/**
 * Step-by-step triage via tool-using LLM.
 *
 * Each alert has a playbook (see llm-tools.ts). The LLM runs ONE step at a
 * time with a narrow toolset, posts its finding, and offers an inline
 * "Scan deeper" button that runs the next step. This mirrors how I manually
 * investigate: check the most suspicious signal first, decide whether to
 * drill down, only spend credits if the first read looks off.
 *
 * Cost model: step 1 alone (usually 1-2 tool calls) is typically enough when
 * the event is routine. Full 3-step playbook only runs if the team clicks
 * through. Beats a static "dump everything at triage time" approach.
 *
 * Run on demand: npx tsx src/llm-triage.ts <protocol> [alertType] [step]
 */
 ...
 * we have seen in the wild rather than trying to be cute about it.
 */
...
/**
 * De-dupe recent triage fires so listener restarts (which replay missed
 * events on reconnect) do not re-burn the LLM budget on the same underlying
 * change.
 *
 * Two windows: a baseline 6-hour bucket per (protocol, programId, type),
 * and a 24-hour bucket for known high-frequency single-signer protocols.
 * Those protocols upgrade routinely and every prior triage has returned
 * "routine"; running 3-4 playbook steps per upgrade is pure LLM waste.
 *
 * If a real change in the underlying setup happens (authority swap,
 * threshold drop, new signer), the listener fires a different alert
 * type that misses the dedupe key and triages immediately.
 */
...
function shouldSuppressTriage(input: TriageInput): boolean {
  ...
  // Opportunistic prune (use the longer window so we don't drop hot keys early)
