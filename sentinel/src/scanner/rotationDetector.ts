...
export async function checkRecentRotation(
  ...
  try {
    ...
    if (hasRecent) {
      ...
      if (mostRecent.daysAgo <= 14) {
        warning =
          `Multisig configuration was changed ${mostRecent.daysAgo} days ago. ` +
          `Drift's multisig was rotated ~10 days before the exploit. ` +
          `Recent rotation combined with other risk factors warrants immediate review.`;
      } else {
        warning =
          `Multisig configuration was changed ${mostRecent.daysAgo} days ago. ` +
          `Monitor for follow-up changes.`;
      }
    }
