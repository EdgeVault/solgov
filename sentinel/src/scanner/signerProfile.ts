// Score signer behaviour patterns across tracked multisigs.
...
export async function profileSigner(
  ...
  // Hot wallet signals
  if (tokenAccounts > 10) {
    indicators.push(`${tokenAccounts} token accounts on this signer`);
  }
  if (interactsWithDefi) {
    indicators.push('Signer interacts with DeFi programs in addition to multisig instructions');
  }
...
export function formatSignerProfiles(profiles: SignerProfile[]): string {
  ...
  if (hotCount > 0) {
    out += `${hotCount} of ${profiles.length} signer wallet(s) show active DeFi or trading interactions in addition to multisig signing. `;
    out += `Squads' published operational guidance recommends dedicated signing devices for multisig roles.\n\n`;
  }

  if (coldCount === profiles.length) {
    out += `All signer wallets show interaction patterns consistent with dedicated signing use.\n\n`;
  }
