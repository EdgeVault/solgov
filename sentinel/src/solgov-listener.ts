...
async function processV4State(name: string, ms: any) {
  try {
    ...
    if (changes.length > 0) {
      ...
      const addedSigners = memberKeys.filter(k => !prev.memberKeys.includes(k));
      const removedSigners = prev.memberKeys.filter(k => !memberKeys.includes(k));
...
async function handleProgramUpgrade(name: string, conn: Connection, programId: string) {
  ...
  try {
    ...
    const singleSignerProgs = ['BisonFi', 'HumidiFi', 'Photon', 'Save'];
    const isSingleSigner = singleSignerProgs.some(s => name.includes(s));
    ...
    if (isSingleSigner) {
      ...
      header = isAuthorityChange
        ? '🔴 <b>CRITICAL: Single-signer program authority changed</b>'
        : '🔴 <b>CRITICAL: Single-signer program upgrade</b>';
    ...
    if (severity === 'CRITICAL' || severity === 'HIGH') {
      ...
      const pubAction = isAuthorityChange ? 'Upgrade authority changed.' : 'Program upgraded.';
      const pub = `<b>${name}</b>\n${pubAction}\nProgram: <code>${programId.slice(0, 12)}...</code>\n${authLine}\n${timestamp} UTC\nsolgov.xyz`;
      await sendPublic(pub);
    } else {
      await sendTelegram(msg, 'MONITOR');
