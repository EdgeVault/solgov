    ...
    for (const sig of recent.slice(0, 5)) {
      ...
      if (nonceIxType) {
        ...
        alerts.push({
          ...
          precedent: 'Drift ($285M): nonces created 9 days before exploit',
        });
        break;
      }
      ...
      for (const ix of allIx) {
        ...
        if (bridgeName && signerInitiated) {
          alerts.push({
            ...
            detectedAt: time, signature: sig.signature.slice(0, 20),
            precedent: 'Bybit ($1.5B): funds bridged within hours of exploit',
          });
          break;
        }
        ...
        if (prog === 'BPFLoaderUpgradeab1e11111111111111111111111' && signerInitiated) {
          alerts.push({
            ...
            detectedAt: time, signature: sig.signature.slice(0, 20),
            precedent: 'Cork ($12M): malicious market deployed via admin key',
          });
          break;
        }
      }
...
async function main() {
  ...
  if (isReport) {
    ...
    if (lowThreshold.length > 0) {
      msg += `\n⚠️ <b>Low threshold (${lowThreshold.length})</b>\n${lowThreshold.join('\n')}\n`;
    }
