/**
 * Per-user subscription state for solgov-bot + solgov-listener.
 *
 * Flat JSON file at sentinel/data/bot-subscriptions.json. Both the bot
 * (command handler, writes) and the listener (broadcast router, reads)
 * load from the same file. Shape is tiny and easy to audit by hand.
 */
