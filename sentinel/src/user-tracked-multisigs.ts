/**
 * Registry of user-submitted Squads v4 multisigs that the listener watches
 * in addition to the curated 53-protocol set.
 *
 * Closes the one remaining functional gap vs. multisig-monitor (Squads):
 * any wallet or builder can submit a Squads v4 multisig address and start
 * receiving alerts via Telegram subscriptions or the public webhook API,
 * without self-hosting.
 *
 * Storage: sentinel/data/user-tracked-multisigs.json (gitignored runtime).
 *
 * Subscriptions for the multisig are still per-user via the existing
 * subscription system; the address joining the watch list is a separate,
 * shared piece of state. When the last subscriber unsubscribes, the
 * address is left in the watch list (negligible cost: one accountSubscribe
 * stream per address) until manually cleaned up.
 */
 ...
 * before we accept it. This is the critical step - anyone with a string
 * could submit junk; we only add things that decode as Squads accounts.
 *
